...
from doc_chunking.schemas import LayoutExtractionResult, LayoutElement, ElementType
...
def display_layout(layout_extraction_result: LayoutExtractionResult, 
    ...
    for element in layout_extraction_result.elements:
        ...
        try :
            ...
            logger.error(f"Error displaying layout element: {e}")
            continue
...
class DisplayLine(BaseModel):
    ...
    def from_layout_element(cls, layout_element: LayoutElement) -> "DisplayLine":
        ...
        return cls(
            ...
            font_name=layout_element.style.runs[0].font.name if layout_element.style.runs and layout_element.style.runs[0].font.name else "Unknown" if layout_element.style else None,
            font_size=round(layout_element.style.runs[0].font.size, 1) if layout_element.style.runs and layout_element.style.runs[0].font.size else 12.0 if layout_element.style else None,
            font_color=layout_element.style.runs[0].font.color if layout_element.style.runs and layout_element.style.runs[0].font.color and layout_element.style.runs[0].font.color != '#000000' else None if layout_element.style else None,
            font_italic=layout_element.style.runs[0].font.italic if layout_element.style.runs and layout_element.style.runs[0].font.italic else None if layout_element.style else None,
            font_underline=layout_element.style.runs[0].font.underline if layout_element.style.runs and layout_element.style.runs[0].font.underline else None if layout_element.style else None,
            font_bold=layout_element.style.runs[0].font.bold if layout_element.style.runs and layout_element.style.runs[0].font.bold else None if layout_element.style else None,
            alignment=layout_element.style.paragraph_format.alignment.value if layout_element.style.paragraph_format else None if layout_element.style else None,
